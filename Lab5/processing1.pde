...
float value = 0;                  // value from sensor
int motion = 0;                 // value to store which way the servo is panning
float[] newValue = new float[181];  // create an array to store each new sensor value for each servo position
float[] oldValue = new float[181];  // create an array to store the previous values.
...
if (degree >= 45) {                  // if at the far right then set motion = 1/ true we're about to go right to left
...
x = value + cos(radians((180+i)))*((oldValue[i])); // create x coordinate
y = value + sin(radians((180+i)))*((oldValue[i])); // create y coordinate
...
text("Distance: "+Float.toString(value), 100, 400, 100, 50);         // text(string, x, y, width, height)
