...
// initializations
//LiquidCrystal (1, 8, 9, 10, 12, 13); // CHECK BEFORE USE
Servo myServo;
...
#define sV 180 // going straight velocity 
#define tV 255 // turning velocity
...
void setup()
{
  ...
  // setup the  screen to a 2x16 (rows x cols) display
...
//-------------------------- setting variables for the function ---------------------
  ...
  if (Mid > 180) { //  check if the middle sensor on the black track
    ...
    // T > 180 consider the R and L sensors are on different colors
      ...
      digitalWrite(M1, HIGH);
      digitalWrite(M2, HIGH);
      // turn left if the right one is on white and stop when the middle one is on black
      if (T1 > T2) {
        if (Mid < 100)
          go(tV, 0);
        else
      ...
      //turn right if the left one is on white and stop when the middle one is on black
      else {
        if (Mid < 100)
          go(0, tV);
        else
...
//the middle one is on white( NOT on the black)
  else {
      //all three sensors are NOT on the black line, kee moving straight
    if (T <= 180) {
      digitalWrite(M1, HIGH);
      digitalWrite(M2, HIGH);
      go(sV, sV);
    }
    //turn if two differences are very different
    else {
      digitalWrite(M1, HIGH);
      digitalWrite(M2, HIGH);
      //turn left if the left one is on the black
      if (T2 > T1) {
          //keep turning until the middle one is on black
        if (Mid < 100)
          go(tV, 0);
        else
          go(0, 0);
      }
      //turn right if the right one is on black
      else {
          //keep turning until the middle one is on black
        if (Mid < 100)
          go(0, tV);
...
//control the velocities of two wheels
