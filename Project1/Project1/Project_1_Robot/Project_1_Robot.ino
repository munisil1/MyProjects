...
// initializations
LiquidCrystal lcd(8, 9, 10, 11, 12, 13); // CHECK BEFORE USE
Servo myServo;
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(R_HALL_EFF), wheelSpeedR, FALLING); // WARNING: may have concurrency issues
  attachInterrupt(digitalPinToInterrupt(L_HALL_EFF), wheelSpeedL, FALLING); // WARNING: may have concurrency issues
}
...
void loop() {
  ...
  if (!stopped) {
    analogSpeedOfRobot = map(constrain(distance, 50, 20), 50, 20, 240, 120); // NOTE: may need to change mapping when switching over to batteries
    lcd.setCursor(0, 0);
...
//  analogWrite(E2, analogSpeedOfRobot + adjustL); // PWM control with speed adjustments
    digitalWrite(M1, HIGH);
    //analogWrite(E1, BASE_SPEED + adjustR);
    analogWrite(E1, analogSpeedOfRobot + adjustR);
    digitalWrite(M2, HIGH);
    //analogWrite(E2, ((BASE_SPEED + adjustL) * 105) / 100);
    analogWrite(E2, analogSpeedOfRobot + adjustL) * 105) / 100);
...
void turnLeft() {
  ...
  analogWrite(E1, 255);
  analogWrite(E2, 0);
  delay(1300); // TODO: change delay when robot is fully functional
  // disable both wheels
  analogWrite(E1, 0);
  analogWrite(E2, 0);
  delay(100);
...
void turnRight() {
  ...
  analogWrite(E1, 0);
  analogWrite(E2, 255);
  delay(1550); // TODO: change delay when robot is fully functional
  // disable both wheels
  analogWrite(E1, 0);
  analogWrite(E2, 0);
  delay(100);
...
void calcSpeedAdjust() {
  if (abs(currSpeedL - currSpeedR) > 0.5) {
    if (currSpeedR > currSpeedL && (((analogSpeedOfRobot + adjustL) * 105) / 100) < MAX_SPEED && analogSpeedOfRobot + adjustR < MAX_SPEED) {
      adjustL += 1;
    } else if (currSpeedL > currSpeedR && (((analogSpeedOfRobot + adjustL) * 105) / 100) < MAX_SPEED && analogSpeedOfRobot + adjustR < MAX_SPEED) {
      adjustR += 1;
    }
  } else {
